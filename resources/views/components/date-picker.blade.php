...
<div x-data="datePicker('{{ $name }}')">
    ...
    <div class="relative" x-ref="inputWrapper">
        <div @click="toggleCalendar" class="flex items-center border rounded-md py-2 px-3 border-gray-300 bg-white cursor-pointer dark:border-gray-600 dark:bg-gray-800">
            <input x-model="selectedDate" type="text" readonly placeholder="YYYY-MM-DD" class="w-full bg-transparent outline-none text-gray-900 dark:text-gray-50 text-sm select-none cursor-pointer" />
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-700 dark:text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
        ...
        <!-- Calendar Dropdown - will be moved to body by JavaScript -->
        <div x-ref="calendar" x-show="showCalendar" x-cloak class="fixed border border-gray-300 mt-1 bg-white rounded-md p-5 dark:bg-gray-800 dark:border-gray-600" style="z-index: 99999; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.3), 0 10px 10px -5px rgba(0, 0, 0, 0.2); display: none;">
            ...
            <div class="flex justify-end mt-2">
                <button @click="selectToday" type="button" class="bg-gray-700 px-2 py-1 text-white rounded-sm text-xs hover:bg-gray-800">Tomorrow</button>
            </div>
        </div>
    </div>
